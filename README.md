...
For a previous exploration of these issues please see my paper, Structures of Identity:
	https://github.com/WebOfTrustInfo/rwot8-barcelona/raw/master/topics-and-advance-readings/structures_of_identity.pdf	
...
A good way of exploring how Nodemail works is by working through the basic concepts, and then looking at several use scenarios:
	...
	'Where' is Nodemail
		...
		Federation versus decentralization
		Implementation considerations

	Two party communication (1-to-1)

		Nodemail-to-Email
		Nodemail-to-Nodemail

	Multiparty Communication

		Hybrid Multiparty (including Email users)
		Nodemail multiparty
...
Alice has unique addresses for each connection she has: 

```
                           | --> A(D) <--> D    <-- Dan's Email          <-- Dan
Alice --> Alice's Nodemail | --> A(B) <--> B(A) <-- Bob's Nodemail       <-- Bob
                           | --> A(C) <--> C    <-- Corporation's Email  <-- Bob
```

## Nodes

A node address is an email address which looks like this:

	hNKgdSNrdWSdFl9emSX8IQWvP9coh@nodemail.org

Which is shorthand for:

	did:ndml:hNKgdSNrdWSdFl9emSX8IQWvP9coh@nodemail.org

A node is comprised of both the CID / hash link to a Decentralized Identifier document (DID) stored on IPFS, and the document itself, which contains the following information:

	Timestamp
	Nodemail Protocol Resolving Server Domain(s)
	Public Key (Type 2 HID Child Key of Parent)

The hash of which constitutes the address. This guarantees the integrity of the timestamp and public key, irrespective of resolver domain. 
...
Most modern email clients display "pet names" i.e. Alice C. instead of AliceC@domain.com, so the actual node address is not necessary for Alice to remember, thank goodness. 
...
## Communicating with a corportation over Email
...
In general, when both individuals are using Nodemail, they can expect features more akin to Signal than email, i.e. E2E encryption, no need to interact with hashes or keys, and automatic verification of message integrity. 
